...
class SAC(object):
    ...
    def update_parameters(self, memory, batch_size=16, weights=None):
        self.actor.train()

        # Sample from the experience replay buffer (supports both uniform and PER)
        sample = memory.sample(batch_size=batch_size)
        per_indices = None
        if len(sample) == 7:   # PrioritizedReplayBuffer returns (s,a,ns,r,d,indices,weights)
            state_batch, action_batch, next_state_batch, reward_batch, mask_batch, per_indices, per_weights = sample
            if weights is None:
                weights = per_weights
        else:
            state_batch, action_batch, next_state_batch, reward_batch, mask_batch = sample
        ...
        # Compute per-sample TD errors (used to update PER priorities)
        with torch.no_grad():
            td_errors = ((torch.abs(qf1 - next_q_value) + torch.abs(qf2 - next_q_value)) / 2.0).squeeze().cpu().numpy()

        # Compute the critic loss  (weighted by IS weights if using PER)
