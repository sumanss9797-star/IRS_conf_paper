...
class Beta_Space_Exp_SAC(object):
    ...
    def update_parameters(self, memory, exp_regularization, batch_size=16, weights=None):
        self.actor.train()

        # Sample from the experience replay buffer (supports both uniform and PER)
        sample = memory.sample(batch_size=batch_size)
        per_indices = None
        if len(sample) == 8:   # BetaPrioritizedReplayBuffer: (s,a,beta,ns,r,d,indices,weights)
            state_batch, action_batch, beta_batch, next_state_batch, reward_batch, mask_batch, per_indices, per_weights = sample
            if weights is None:
                weights = per_weights
        else:
            state_batch, action_batch, beta_batch, next_state_batch, reward_batch, mask_batch = sample
        ...
        # Critic loss (IS-weight-corrected when using PER)
